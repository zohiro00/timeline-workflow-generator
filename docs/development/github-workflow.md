...
- 正規ルートの開始前に `gh auth status` を実行しません。GitHub コネクタによる PR 作成に GitHub CLI の認証確認は不要です。
- `gh auth status` は、正規ルートが使えず CLI fallback へ切り替えることを決めた後にだけ、権限付きで実行します。
...
この節は `gh` または `pnpm run pr:create` を使う fallback に切り替えた場合だけ適用します。正規ルートの事前確認としては実行しません。
...
正規ルートと fallback の境界は次のとおりです。

1. PR 作成依頼を受けた時点では、GitHub コネクタを選びます。`gh auth status` は実行しません。
2. ローカル Git で commit と push を行います。
3. GitHub コネクタで draft PR を作成します。
4. コネクタが利用不能、権限不足、またはユーザーが CLI を明示した場合にだけ fallback を選びます。
5. fallback を選んだ後、権限付きで `gh auth status` を確認してから `pnpm run pr:create` または `gh pr create` を使います。
...
- 正規ルートの preflight として `gh --version` や `gh auth status` を要求しないでください。これらは CLI fallback の前提条件です。

## やってはいけないこと

- `gh` と GitHub コネクタの両方を場当たり的に試し、失敗理由を混ぜないでください。
- `gh auth status` の sandbox 失敗を、認証情報そのものの破損として報告しないでください。
- GitHub コネクタを試す前に、CLI fallback の認証失敗を理由として PR 作成を止めないでください。
