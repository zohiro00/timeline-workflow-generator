...
node label は `<br>` で明示改行できます。
SVG に描画されるのは `## workflow` の依存関係で参照されたノードだけです。

### ノードを強調する

図の中で最も注目してほしいノードには、ノードIDの直後に `[highlight]` を指定できます。

```workflow
## nodes
- reception
  - a1 [highlight]: 問い合わせ受付
  - a2: 一次確認
```

`[highlight]` を指定できるのは、1つのワークフローにつき最大1ノードです。
強調指定のない既存のノード表示や、依存関係から計算される配置には影響しません。
強調したノードも、`## workflow` から参照されていない場合は描画されません。
...
1. Engineでワークフローを書く
2. プレビューで図を確認する
3. ExportからPPTX / SVG / PNGをダウンロードする、または画像としてコピーする
4. PPTXはPowerPointで開き、ノード図形と標準コネクタとして編集する
5. SVG / PNG / コピー画像はPowerPointへ貼り付け、スライド上でサイズを調整する
